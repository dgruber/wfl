	...
	ctx := googlebatch.NewGoogleBatchContextByCfg(
		googlebatch.Config{
			DefaultTemplate: drmaa2interface.JobTemplate{
				JobCategory:       "gcr.io/" + GoogleProject + "/bark-googlebatch",
...
var getJobOutput = func(j drmaa2interface.Job, i interface{}) error {
	...
	jobID := j.GetID()
	if !slices.Contains(trainingJobIDs, jobID) {
		return nil
	}
	...
	if j.GetState() == drmaa2interface.Failed {
		...
		out, err := gcpbatchtracker.ReadFromBucket("gs://"+GCPBucketName,
			jt.JobEnvironment["LOGFILE"])
		if err != nil {
			return err
		}
